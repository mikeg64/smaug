...
if (!d.flags and 1) ne 0 then begin
	...
	mm=max([abs(mina),abs(maxa)]) 
     ...
     endelse
     if keyword_set(bar) then begin
        barim=findgen(1,255)/255*(maxa-mina)+mina
        if keyword_set(noscale) then begin
...
;	mm=max([abs(mina),abs(maxa)])     
	mm=max([abs(min(b)),abs(max(b))])     	
	     bb=b
;      if (mm ge 0.06) then begin

        for i=0,sx-1 do begin
	 for j=0, sy-1 do begin
;	  if b(i,j) lt -0.1d-5 then begin
;	    bb(i,j)=127*(b(i,j)+mm)/mm+1
;	  endif else begin
;   	    bb(i,j)=127+127*b(i,j)/mm
;	  endelse
  	    bb(i,j)=128.d0+127.d0*b(i,j)/mm
