...
window, 4,xsize=800,ysize=450,XPOS = 300, YPOS = 500 
;***************
headline='                                                                               '
it=long(1)
ndim=long(1)
...
tvframe, rotate(reform(bx(hh,*,*)),1)*1.0e4, title='Bx', /bar,  CT='rho0',$
         xtitle='x [Mm]', ytitle='y [Mm]',charsize=cs, $ 
         xrange=[min(y)/scale, max(y)/scale], $
	 yrange=[min(x)/scale, max(x)/scale]

tvframe, rotate(reform(xf(*,*,yy)),1), title='xf', /bar,  CT='rho0', $
         xtitle='x [Mm]', ytitle='z [Mm]',charsize=cs, $ 
         xrange=[min(x)/scale, max(x)/scale], $
	 yrange=[min(z)/scale, max(z)/scale]

tvframe, rotate(reform(by(hh,*,*)),1)*1.0e4, title='By', /bar,  CT='rho0',$
         xtitle='x [Mm]', ytitle='y [Mm]',charsize=cs, $ 
         xrange=[min(x)/scale, max(x)/scale], $
	 yrange=[min(y)/scale, max(y)/scale]
	 
tvframe, rotate(reform(by(*,yy,*)),1)*1.0e4, title='By', /bar,  CT='rho0',$
         xtitle='x [Mm]', ytitle='z [Mm]',charsize=cs, $ 
         xrange=[min(x)/scale, max(x)/scale], $
	 yrange=[min(z)/scale, max(z)/scale]

tvframe, rotate(reform(bz(*,*,yy)),1)*1.0e4, title='Bz', /bar,  CT='rho0',$
         xtitle='x [Mm]', ytitle='z [Mm]',charsize=cs, $ 
         xrange=[min(x)/scale, max(x)/scale], $
	 yrange=[min(z)/scale, max(z)/scale]
...
tvframe, rotate(reform(bx(*,*,yy)),1)*1.0e4, title='Bx', /bar,  CT='rho0',$
         xtitle='x [Mm]', ytitle='z [Mm]',charsize=cs, $ 
         xrange=[min(x)/scale, max(x)/scale], $
	 yrange=[min(z)/scale, max(z)/scale]

image_p = TVRD_24()
write_png,'/data/ap1vf/png/3D/tube/mag_field.png',image_p, red,green, blue
