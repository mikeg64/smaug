...
for j=0,n2-1 do begin
for i=0,n1-1 do begin	
	dpdz(i,j)=ggg*w(i,j,0)-w(i,j,4)*dbzdz(i,j)-w(i,j,5)*dbzdx(i,j)-dbsq(i,j)/2;	
endfor
endfor
...
for i=0,n1-1 do begin
  for j=0,n2-1 do begin
     w(i,j,6)=((p(i,j)-(bsq(i,j)/2))/(gamma-1))+(bsq(i,j)/2)
     w(i,j,3)=0.0
 endfor
endfor
