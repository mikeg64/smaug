...
;openr,1,'/data/ap1vf/2_6MmBz252.ini',/f77_unf
;openr,1,'/data/ap1vf/2_6Mnzx1023400.ini',/f77_unf

openr,1,'/data/ap1vf/2_6Mnzx1976400.ini',/f77_unf
...
for i=0,n1-1 do b0z(i)=(par4((z[i]-max(z)/2.d0)+max(z)/2.d0,8.0d5,1.d0))^2.d0
for i=n1-2,0,-1 do b0z(i)=b0z(i)+b0z(i+1)

Bmax=0.00415d0  ; Tesla bottom

b0z=Bmax*b0z/max(b0z)

;b0z(*)=b0z(*)+0.00035d0  ; + Tesla top

b0z(*)=b0z(*)+0.00031d0  ; + Tesla top
...
xf=(par3(b0z(j)*x(i),0.5d3,0.5d0))^2.d0
...
for i=0,n2-1 do begin
 dbzdz(*,i)=deriv(z,bz(*,i))
 dbxdz(*,i)=deriv(z,bx(*,i))
endfor

for i=0,n1-1 do begin
 dbxdx(i,*)=deriv(x,bx(i,*))
 dbzdx(i,*)=deriv(x,bz(i,*))
endfor
...
for i=0,n2-1 do dbxbzdz(*,i)=deriv16(bxbz(*,i),z)

;dbxbzdz=SMOOTH(dbxbzdz, 20)

for i=0,n2-1 do dbxbzdzdz(*,i)=deriv16(dbxbzdz(*,i),z)

intBzBx=dblarr(n1,n2)


for j=0,n1-1 do begin
print,j
  for i=0,n2-1 do begin
   sum=inte(reform(dbxbzdz[j,0:i]),x(1)-x(0))
;    sum=int_tabulated(x[0:i],reform(dbxbzdx[j,0:i]),/double)  
   intBzBx(j,i)=sum
 endfor
endfor
...
;intBzBx=SMOOTH(intBzBx, 20)



p=(bx^2.d0-bz^2.d0)/2.d0+intBzBx

nn1=1

;p=congrid(p,n1*nn1,n2)
;z=congrid(z,n1*nn1)

dpdz=dblarr(n1*nn1,n2)

for i=0,n2-1 do dpdz(*,i)=deriv1(p(*,i),z)

;dpdz=congrid(dpdz,n1,n2)
;p=congrid(p,n1,n2)
;z=congrid(z,n1)

rho1=(dpdz+G)*(1.d0/ggg)
...
close,1
openw,1,'/data/ap1vf/2_6Mnzx1976400B41.ini',/f77_unf
writeu,1,headline
