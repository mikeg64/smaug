...
window, 3,xsize=300,ysize=300,XPOS = 300, YPOS = 500 
;***************
headline='                                                                               '
it=long(1)
ndim=long(1)
...
plot, z(*)/1.0d6,alog10(p(*,49,49)), title='alog(p_gas)', charsize=1.2, xtitle='[Mm]'
...
x=x-max(x)/2.d0+5000.d0
y=y-max(y)/2.d0+5000.d0


d_z=0.5d0 ; width of Gaussian in Mm
z_shift= 0.2 ; shift in Mm
A=0.45d0 ; amplitude

;for i=0,n1-1 do b0z[i]=(par3((z[i]/scale-z_shift),d_z,A))^2.d0
...
xf[i,j,k]=(par4(f,1.0d8,0.1d0))^2.d0
...
for k=0,n3-1 do begin 
for j=0,n2-1 do begin
 ax[j,k]=x[j]/sqrt(x[j]^2.d0+y[k]^2.d0)
 ay[j,k]=y[k]/sqrt(x[j]^2.d0+y[k]^2.d0)
 az[j,k]=1.d0/sqrt(x[j]^2.d0+y[k]^2.d0) 
endfor
endfor
ax[49,49]=0.d0
ay[49,49]=0.d0
...
bz(i,j,k)=az[j,k]*b0z[i]*xf[i,j,k]
bx(i,j,k)=-ax[j,k]*dbz[i]*xf[i,j,k]
by(i,j,k)=-ay[j,k]*dbz[i]*xf[i,j,k]

endfor
endfor
endfor

bz(*,49,49)=b0z(*)/10000.d0
...
tvframe, by(*,50,*), title='by'
tvframe, bz(*,*,50), title='bz'
...
Bvarix1=dblarr(n1,n2,n3)
Bvarix2=dblarr(n1,n2,n3)

Bvariy=dblarr(n1,n2,n3)
Bvariy1=dblarr(n1,n2,n3)
Bvariy2=dblarr(n1,n2,n3)
...
for k=0,n3-1 do begin
  for j=0,n2-1 do begin
   sum=inte(reform(Bvar[i,0:j,k]),x(1)-x(0)) 
  Bvarix1(i,j,k)=sum
...
for j=0,n2-1 do begin
  for k=0,n3-1 do begin
   sum=inte(reform(Bvar[i,j,0:k]),y(1)-y(0)) 
  Bvariy1(i,j,k)=sum
 endfor
endfor
print, 'Iy1 ', i
...
for j=0,n2-1 do begin
for i=0,n1-1 do begin
 dBxBydy(i,j,*)=deriv1(Bvarix1(i,j,*),y)
endfor
endfor

for j=0,n2-1 do begin
for k=0,n3-1 do begin
 dBxBzdz(*,j,k)=deriv1(Bvarix2(*,j,k),z)
endfor
endfor

for j=0,n2-1 do begin
for i=0,n3-1 do begin
 dBxBydx(i,j,*)=deriv1(Bvariy1(i,j,*),x)
endfor
endfor

for j=0,n2-1 do begin
for k=0,n3-1 do begin
 dByBzdz(*,j,k)=deriv1(Bvariy2(*,j,k),z)
endfor
endfor

Bvarix=dBxBydy+dBxBzdz
Bvariy=dBxBydx+dByBzdz
...
;for j=0,n2-1 do begin
;  for k=0,n3-1 do begin
;   sum=inte(reform(G[i,j,0:k]),y(1)-y(0)) 
;  Bvariy(i,j,k)=sum
; endfor
;endfor
;print, 'Iy ', i
