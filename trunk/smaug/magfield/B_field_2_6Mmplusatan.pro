...
nn=0
close,1
openr,1,'/data/ap1vf/2_6MmBz1023.ini',/f77_unf
readu,1,headline
...
ff=dblarr(n2,n1)
for i=0,n1-1 do b0z(i)=(par4(i+0.4*n1/2,580.d0,0.5e-2))^2.d0
...
;*************for p
;b0z(*)=sqrt(p(*,200))/1.5d4+0.00038d0
...
xf=(par3(b0z(j)*x(i),0.4d3,0.5d0))^2.d0
...
for j=0,n1-1 do begin
print,j
  for i=3,n2-1 do begin
;   sum=inte(reform(F[j,0:i]),x(1)-x(0))
    sum=int_tabulated(x[0:i],reform(F[j,0:i]),/double)  
...
for i=0,n2-1 do intFdz(*,i)=deriv(z,bvari(*,i))/ggg
...
plot, rho1(490:503,200)
oplot, rho1(490:503,200), psym=4

plot, e(490:503,200)
oplot, e(490:503,200), psym=4

qq=dbzdz+dbxdx

close,1
openw,1,'/data/ap1vf/2_6MmBT.ini',/f77_unf
writeu,1,headline
